...
function MethodDungeonTools:MakeSidePanel(frame)
	...
	frame.LinkToChatButton:SetCallback("OnClick",function(widget,callbackName,value)
        ...
        frame.LinkToChatButton:SetText("发送")
    ...
    affixWeekWarning:SetCallback("OnEnter",function(...)
        ...
        GameTooltip:SetText("选定的词缀不是本周的词缀")
        GameTooltip:Show()
    end)
	...
	frame.sidePanel.DifficultySliderLabel:SetText("层数: ")
...
function MethodDungeonTools:DisplayMDISelector()
    ...
    if not MethodDungeonTools.MDISelector then
        ...
        MethodDungeonTools.MDISelector.BeguilingDropDown:SetLabel("迷醉:")
        local beguilingList = {"1. 虚空","2. 潮汐","3. 魔法"}
			...
			for k,v in pairs(frame.sidePanel.newPullButtons[tooltip.currentPull].enemyPortraits) do
				if MouseIsOver(v) then
					if v:IsShown() then
                        ...
                        text = text.."进度: "..MethodDungeonTools:FormatEnemyForces(v.enemyData.count,totalForcesMax,false)
...
function MethodDungeonTools:FormatEnemyHealth(amount)
    ...
    elseif amount >= 1e8 then
        return string.format("%.3f亿", amount/1e8)
...
function MethodDungeonTools:CreateTutorialButton(parent)
	...
	local helpPlate = {
		...
		[1] = { ButtonPos = { x = 190,	y = 0 }, HighLightBox = { x = 0, y = 0, width = 197, height = 56 },		ToolTipDir = "RIGHT",		ToolTipText = "选择副本" },
		[2] = { ButtonPos = { x = 190,	y = -210 }, HighLightBox = { x = 0, y = -58, width = sizex-6, height = sizey-58 },	ToolTipDir = "RIGHT",	ToolTipText = "选择要拉的怪\nCTRL+左键选择单一怪" },
		[3] = { ButtonPos = { x = 828,	y = 0 }, HighLightBox = { x = 838, y = 30, width = 251, height = 87 },	ToolTipDir = "LEFT",	ToolTipText = "管理预设" },
		[4] = { ButtonPos = { x = 828,	y = -87 }, HighLightBox = { x = 838, y = 30-87, width = 251, height = 83 },	ToolTipDir = "LEFT",	ToolTipText = "自订副本选项" },
		[5] = { ButtonPos = { x = 828,	y = -(87+83) }, HighLightBox = { x = 838, y = 30-(87+83), width = 251, height = 415 },	ToolTipDir = "LEFT",	ToolTipText = "创建和管理拉怪\n右键来获得更多选项" },
	}
