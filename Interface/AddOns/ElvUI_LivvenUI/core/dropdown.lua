...
local DD = E:NewModule("LuiDropdown", "AceEvent-3.0")
...
local function CreateListButton(frame)
    local button = T.CreateFrame("Button", nil, frame, "SecureActionButtonTemplate")
    ...
    button.hoverTex:SetBlendMode("BLEND")
    button.hoverTex:SetDrawLayer("BACKGROUND")
...
function LUI:DropDown(list, frame, MenuAnchor, FramePoint, xOffset, yOffset, parent, customWidth, justify)
    if T.InCombatLockdown() then return end
    if not frame.buttons then
        frame.buttons = {}
        frame:SetFrameStrata("TOOLTIP")
        frame:SetClampedToScreen(true)
        T.table_insert(UISpecialFrames, frame:GetName())
        frame:Styling()
        frame:Hide()
    end
        ...
        for i = 1, #list do
            ...
            if list[i].secure then
                ...
                elseif btn.secure.buttonType == "macro" then
                    ...
                    LUI:Print("Wrong argument for button type: " .. btn.secure.buttonType)
                end
            end
...
function DD:GetCooldown(CDtype, id)
    ...
    if start > 0 then
        ...
        cd = T.string_format(E.TimeFormats[formatID][2], cd)
        return cd
    end
    return nil
end
...
function DD:RegisterMenu(menu)
    ...
    if name then
        ...
        LUI:Print("Dropdown not registered. Please check if it has a name.")
    end
end
