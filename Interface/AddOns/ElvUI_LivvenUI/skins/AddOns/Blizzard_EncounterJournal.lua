...
function LUI:StyleEncounterJournal()
	if E.private.skins.blizzard.enable ~= true or E.private.skins.blizzard.encounterjournal ~= true or E.private.lui.luiSkins.blizzard.encounterjournal ~= true then return end
	...
	local function styleSearchButton(result, index)
		...
		if result.icon then
			...
			local bg = LS:CreateBG(result.icon)
	...
	local function listInstances()
		...
		while true do
			...
			local bg = LS:CreateBG(bu.bgImage)
	...
	LS:CreateBDFrame(_G.EncounterJournalEncounterFrameInfoModelFrame, .25)
	...
	hooksecurefunc(EncounterJournal.LootJournal.ItemSetsFrame, "ConfigureItemButton", function(_, button)
		if not button.bg then
			...
			button.bg = LS:CreateBDFrame(button.Icon)
		end
	...
	do
		...
		LS:CreateBD(suggestion, .25)
		LS:CreateGradient(suggestion)
	...
	hooksecurefunc("EJSuggestFrame_UpdateRewards", function(suggestion)
		...
		if rewardData then
			...
			if not suggestion.reward.icon.backdrop then
				LS:CreateBackdrop(suggestion.reward.icon)
...
S:AddCallbackForAddon("Blizzard_EncounterJournal", "LuiEncounterJournal", LUI.StyleEncounterJournal)
