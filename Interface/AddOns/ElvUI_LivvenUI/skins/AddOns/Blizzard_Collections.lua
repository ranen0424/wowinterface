...
local function styleCollections()
	if
		E.private.skins.blizzard.enable ~= true or E.private.skins.blizzard.collections ~= true or
			E.private.lui.luiSkins.blizzard.collections ~= true
	 then
		return
	end
	...
	LS:CreateBD(MountJournal.MountCount, .25)
	LS:CreateBD(PetJournal.PetCount, .25)
	LS:CreateBD(MountJournal.MountDisplay.ModelScene, .25)

	-- Mount list
	for _, bu in pairs(MountJournal.ListScrollFrame.buttons) do
		LS:CreateGradient(bu.backdrop)
	...
	card.PetInfo.icon.bg = LS:CreateBG(card.PetInfo.icon)
	...
	LS:CreateBDFrame(card.xpBar, .25)

	for i = 1, 6 do
		local bu = card["spell" .. i]
		LS:ReskinIcon(bu.icon)
	end
	...
	for i = 1, 3 do
		...
		bu.icon.bg = LS:CreateBDFrame(bu.icon, .25)
		...
		LS:CreateBDFrame(bu.healthFrame.healthBar, .25)
	...
	for i = 1, 18 do
		...
		LS:StyleButton(button)
		LS:ReskinIcon(button.iconTexture)
		LS:ReskinIcon(button.iconTextureUncollected)

		button.name:SetPoint("LEFT", button, "RIGHT", 9, 0)

		local bg = LS:CreateBDFrame(button)
	...
	for index = 1, 2 do
		...
		tab.bg = LS:CreateBDFrame(tab, .25)
	...
	LS:CreateBDFrame(SetsCollectionFrame.Model, .25)
	...
	for i = 1, #ScrollFrame.buttons do
		...
		LS:CreateBDFrame(bu.SelectedTexture, .25)
	end
	...
	hooksecurefunc(
		...
		function(_, itemFrame)
			...
			if not ic.bg then
				...
				itemFrame.IconBorder.Show = LUI.dummy
				ic.bg = LS:CreateBDFrame(ic)
			end
	...
	LS:CreateBDFrame(_G.WardrobeOutfitFrame, .25)
	LS:CreateSD(_G.WardrobeOutfitFrame, .25)
...
S:AddCallbackForAddon("Blizzard_Collections", "LuiCollections", styleCollections)
