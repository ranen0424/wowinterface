...
local function styleGarrison()
	...
	-- [[ Garrison system ]]
	function LUI:ReskinMissionPage(self)
		self:StripTextures()
		self.StartMissionButton.Flash:SetTexture("")
		LS:Reskin(self.StartMissionButton)
		...
		local bg = LS:CreateBDFrame(self.Stage)
		...
		for i = 1, 3 do
			...
			LS:CreateBD(follower, .25)
			LS:ReskinGarrisonPortrait(follower.PortraitFrame)
	...
	function LUI:ReskinMissionTabs(self)
		for i = 1, 2 do
	...
	function LUI:ReskinMissionComplete(self)
		...
		LS:CreateBD(bonusRewards)
		LS:Reskin(missionComplete.NextMissionButton)
	end

	function LUI:ReskinGarrMaterial(self)
		self.MaterialFrame.Icon:SetTexCoord(unpack(E.TexCoords))
		self.MaterialFrame:GetRegions():Hide()
		local bg = LS:CreateBDFrame(self.MaterialFrame, .25)
		bg:SetPoint("TOPLEFT", 5, -5)
		bg:SetPoint("BOTTOMRIGHT", -5, 6)
	end

	function LUI:ReskinMissionFrame(self)
		...
		for i = 1, 3 do
			local tab = _G[self:GetName().."Tab"..i]
			if tab then LS:ReskinTab(tab) end
		end
		if self.MapTab then self.MapTab.ScrollContainer.Child.TiledBackground:Hide() end

		LUI:ReskinMissionComplete(self)
		LUI:ReskinMissionPage(self.MissionTab.MissionPage)

		local missionList = self.MissionTab.MissionList
		missionList:StripTextures()
		LUI:ReskinGarrMaterial(missionList)
		LUI:ReskinMissionTabs(missionList)
		hooksecurefunc(missionList, "Update", LUI.ReskinMissionList)

		local followerList = self.FollowerList
		LUI:ReskinGarrMaterial(followerList)
	end

	-- Building frame
	local GarrisonBuildingFrame = _G.GarrisonBuildingFrame
	GarrisonBuildingFrame:StripTextures()
	LS:CreateBD(GarrisonBuildingFrame)
	...
	local bg = LS:CreateBDFrame(BuildingList.MaterialFrame, .25)
	bg:SetPoint("TOPLEFT", 5, -5)
	bg:SetPoint("BOTTOMRIGHT", -5, 6)
	...
	LS:CreateBD(InfoBox, .25)
	LS:CreateBD(TownHallBox, .25)
	LS:Reskin(InfoBox.UpgradeButton)
	LS:Reskin(TownHallBox.UpgradeButton)
	...
	LS:CreateBD(Confirmation)
	LS:Reskin(Confirmation.CancelButton)
	LS:Reskin(Confirmation.BuildButton)
	LS:Reskin(Confirmation.UpgradeButton)
	LS:Reskin(Confirmation.UpgradeGarrisonButton)
	LS:Reskin(Confirmation.ReplaceButton)
	LS:Reskin(Confirmation.SwitchButton)
	...
	LS:CreateBD(GarrisonCapacitiveDisplayFrame.Count, .25)
	...
	LS:Reskin(GarrisonCapacitiveDisplayFrame.StartWorkOrderButton, true)
	LS:Reskin(GarrisonCapacitiveDisplayFrame.CreateAllWorkOrdersButton, true)
	...
	LS:ReskinTab(_G.GarrisonLandingPageTab1)
	LS:ReskinTab(_G.GarrisonLandingPageTab2)
	LS:ReskinTab(_G.GarrisonLandingPageTab3)
	...
	for i = 1, #buttons do
		...
		for _, reward in pairs(button.Rewards) do
			...
			LS:CreateBG(reward.Icon)
			reward:ClearAllPoints()
			reward:SetPoint("TOPRIGHT", -4, -4)
		end

		LS:CreateBD(bg, .25)
		LS:CreateGradient(bg)
	end
	...
	LS:CreateBD(GarrisonMissionFrame, .25)
	LUI:ReskinMissionFrame(GarrisonMissionFrame)
	...
	hooksecurefunc("GarrisonMissionPage_SetReward", function(frame)
		if not frame.bg then
			...
			LS:CreateBDFrame(frame.Icon)
			frame.BG:SetAlpha(0)
			frame.bg = LS:CreateBDFrame(frame.BG, .25)
		...
		for i = 1, #mission.followers do
			...
			if frame.PortraitFrame then
				if not frame.bg then
					...
					oldBg:Hide()
					frame.bg = LS:CreateBDFrame(oldBg)
	...
	LS:Reskin(UnavailableFrame:GetChildren())
	...
	LS:CreateBD(GarrisonRecruitSelectFrame)
	...
	for i = 1, 3 do
		...
		LS:Reskin(recruit.HireRecruits)
	end
	...
	LS:CreateBD(GarrisonMonumentFrame)
	...
	LS:CreateBD(GarrisonShipyardFrame, .25)
	...
	LS:ReskinTab(_G.GarrisonShipyardFrameTab1)
	LS:ReskinTab(_G.GarrisonShipyardFrameTab2)

	local shipyardMission = GarrisonShipyardFrame.MissionTab.MissionPage
	shipyardMission:StripTextures()
	LS:Reskin(shipyardMission.StartMissionButton)

	local smbg = LS:CreateBDFrame(shipyardMission.Stage)
	...
	LS:CreateBD(shipyardMission.RewardsFrame, .25)
	...
	LS:Reskin(GarrisonShipyardFrame.MissionTab.MissionList.CompleteDialog.BorderFrame.ViewButton)
	select(11, GarrisonShipyardFrame.MissionComplete.BonusRewards:GetRegions()):SetTextColor(1, .8, 0)
	LS:Reskin(GarrisonShipyardFrame.MissionComplete.NextMissionButton)
	...
	LS:CreateBD(OrderHallMissionFrame, .25)
	LUI:ReskinMissionFrame(OrderHallMissionFrame)
	...
	LS:CreateBD(_G.OrderHallMissionFrame.MissionTab.ZoneSupportMissionPage, .5)
...
S:AddCallbackForAddon("Blizzard_GarrisonUI", "LuiGarrison", styleGarrison)
