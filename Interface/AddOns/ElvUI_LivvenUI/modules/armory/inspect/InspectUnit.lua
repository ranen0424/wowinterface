...
local ItemLevelPattern = gsub(ITEM_LEVEL, "%%d", "%%.2f")
...
LibEvent:attachTrigger("INSPECT_FRAME_SHOWN", function(self, frame, parent, ilevel)
    if E.db.lui.modules.armory.inspect["ShowInspectAngularBorder"] then
...
LibEvent:attachTrigger("INSPECT_FRAME_COMPARE", function(self, frame)
    ...
    if E.db.lui.modules.armory.inspect["ShowOwnFrameWhenInspecting"] then
