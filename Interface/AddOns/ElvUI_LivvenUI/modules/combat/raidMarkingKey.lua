...
local LRMK = E:NewModule("LuiRaidMarkingKey")

function LRMK:Initialize()
    local menuFrame = T.CreateFrame("Frame", "raidMarkingKey", E.UIParent, "UIDropDownMenuTemplate")
    ...
    WorldFrame:HookScript("OnMouseDown", function(self, button)
        ...
        if E.db.lui.modules.combat.raidMarkers.raidMarkingKey["raidMarkingButton1"] == "alt" and IsAltKeyDown() then
            otherkey = true
        end
        if E.db.lui.modules.combat.raidMarkers.raidMarkingKey["raidMarkingButton1"] == "ctrl" and IsControlKeyDown() then
            otherkey = true
        end
        if E.db.lui.modules.combat.raidMarkers.raidMarkingKey["raidMarkingButton1"] == "shift" and IsShiftKeyDown() then
            otherkey = true
        end
        if button ~= E.db.lui.modules.combat.raidMarkers.raidMarkingKey["raidMarkingButton2"] then
            otherkey = false
        end
        if otherkey and T.UnitExists("mouseover") then
            --if ((T.IsInGroup() and not T.IsInRaid()) or UnitIsGroupLeader("player") or UnitIsGroupAssistant("player")) and not (inInstance and (instanceType == "pvp" or instanceType == "arena")) then
...
local function InitializeCallback()
    if E.db.lui.modules.combat.raidMarkers.raidMarkingKey["raidMarkingButton1"] == "none" then return end
    LRMK:Initialize()
end

E:RegisterModule(LRMK:GetName(), InitializeCallback)
