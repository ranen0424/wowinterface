...
local addonTOCVersion = convertVersionToNumber(GetAddOnMetadata("TomCats-Nazjatar", "version"))
local newFilesSinceVersion = convertVersionToNumber("1.3.8")
if (newFilesSinceVersion > addonTOCVersion) then
    DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Warning: TomCat's Tours: Nazjatar requires that you restart WoW in order for the recent update to function properly|r")
end
...
if (not TomCats) then
    DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Warning: TomCat's Tours must be installed and enabled for TomCat's Tours: Nazjatar to function properly|r")
elseif (tomcatsCurrentVersion < tomcatsMinVersion) then
    DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Warning: TomCat's Tours must be updated to the latest version for TomCat's Tours: Nazjatar to function properly|r")
end
